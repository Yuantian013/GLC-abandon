...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound,):
        ...
        with tf.variable_scope('Actor', reuse=True):
         weight_l4=tf.get_variable('l4/kernel')
        with tf.variable_scope('Actor', reuse=True):
         weight_a=tf.get_variable('a/kernel')

        # a_loss = - tf.reduce_mean(self.q) + self.labda * self.q_lambda+compress_ratio*(tf.norm(weight_l1)+tf.norm(weight_l2)+tf.norm(weight_l3)+tf.norm(weight_l4)+tf.norm(weight_a))  # maximize the q
        a_loss = - tf.reduce_mean(self.q) + self.labda * self.q_lambda \
                 + compress_ratio * (tf.reduce_sum(tf.norm(weight_l1,axis=1))
                                     ...
                                     +  tf.reduce_sum(tf.norm(weight_l4,axis=1))
                 ...
                 + compress_ratio * (tf.reduce_sum(tf.norm(weight_l1,axis=0))
                                     ...
                                     +  tf.reduce_sum(tf.norm(weight_l4,axis=0))
    ...
    def save_result(self):
        ...
        save_path = self.saver.save(self.sess, "Model/Group_V1.ckpt")
