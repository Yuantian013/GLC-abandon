...
LR_D = 0.001    # learning rate for disturb
...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound,):
        ...
        d_cons = self._build_d(self.S_, reuse=True, custom_getter=ema_getter)
        ...
        self.labda_ = self.labda+0.01*tf.reduce_mean(self.q-self.q_cons) #(q-q_+self.R+self.d*self.d)
    ...
    def learn(self,LR_A,LR_C,LR_D,labda):
        ...
        bs_ = bt[:, -self.s_dim:]
        self.sess.run(self.dtrain, {self.S: bs, self.S_: bs_,self.LR_D: LR_D,self.labda:labda})
    ...
    def _build_d(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Disturber', reuse=reuse, custom_getter=custom_getter):
            ...
            return tf.multiply(a, self.a_bound/20, name='scaled_d')

    def save_result(self):
        # save_path = self.saver.save(self.sess, "Save/cartpole_g10_M1_m0.1_l0.5_tau_0.02.ckpt")
        save_path = self.saver.save(self.sess, "Model/SRDDPG_20.ckpt")
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        d = np.clip(np.random.normal(d, var/20), -a_bound/20, a_bound/20)  # add randomness to action selection for exploration
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            if EWMA_reward[0,i+1]>max_ewma_reward:
                ...
                LR_A *= .9  # learning rate for actor
                LR_D *= .9  # learning rate for actor
                LR_C *= .95  # learning rate for critic
                ddpg.save_result()
            ...
            if ep_reward> max_reward:
                ...
                LR_A *= .995
                LR_D *= .995  # learning rate for actor
                LR_C *= .999
            break
