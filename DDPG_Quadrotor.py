...
class DDPG(object):
    ...
    def save_result(self):
        save_path = self.saver.save(self.sess, "Model/Quadrotor.ckpt")
...
a_dim = env.force_space.shape[0]
a_high = env.force_space.high
a_low = env.force_space.low

ddpg = DDPG(a_dim, s_dim, a_high)
ddpg.save_result()
var=0.5*a_high
# var[0]=5*var[0]
print(var)
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        s_, r, done = env.ddpg_step(a[0],a[1:])
        ddpg.store_transition(s, a, r, s_)

        if ddpg.pointer > MEMORY_CAPACITY:
            var= np.multiply( var,.999995)   # decay the action randomness
            #var = np.max([var,0.1])
            # LR_A *= .99995
            # LR_C *= .99995
