...
class DDPG(object):
    ...
    def save_result(self):
        ...
        save_path = self.saver.save(self.sess, "Model/SRDDPG_V3_IN_DREAM.ckpt")
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            print('Episode:', i, ' Reward: %i' % int(ep_reward),' REAL Reward: %i' % int(EP_REWARD), 'Difference : ',abs(1-(ep_reward/EP_REWARD)),'Explore: %.2f' % var,"good","EWMA_step = ",EWMA_step[0,i+1],"EWMA_reward = ",EWMA_reward[0,i+1],"LR_A = ",LR_A,'lambda',labda,'Minimum loss S :',min_loss_s, 'Minimum loss R :',min_loss_r,'LR_D :',LR_D,'LR_R :',LR_R,loss_r)
            ...
            if hit==1:
                print('Episode:', i, ' Reward: %i' % int(ep_reward),' REAL Reward: %i' % int(EP_REWARD),'Difference : ',abs(1-(ep_reward/EP_REWARD)), 'Explore: %.2f' % var, "break in : ", j, "due to ",
