...
class CartPoleEnv_adv(gym.Env):
    ...
    def reset(self):
        ...
        self.state[0] = self.np_random.uniform(low=1, high=4)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            self.target = rendering.Line((4.5 * scale + screen_width / 2.0, 0),
                                         (4.5 * scale + screen_width / 2.0, screen_height))
            self.target.set_color(1, 0, 0)
            self.viewer.add_geom(self.target)


            # Render the constrain position
            self.cons = rendering.Line((3.5 * scale + screen_width / 2.0, 0),
                                         (3.5 * scale + screen_width / 2.0, screen_height))
